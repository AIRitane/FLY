...
int UI_ReFresh(int cnt,...)
{
	...
	else if(robot_state.robot_id == 105)
  {
	...
	else if(robot_state.robot_id == 8)
	{
	datahead.Sender_ID=UI_Data_RobotID_RDark;
	datahead.Receiver_ID=UI_Data_CilentID_RAerial;
	}
	else if(robot_state.robot_id == 108)
  {
		datahead.Sender_ID=UI_Data_RobotID_BDark;
		datahead.Receiver_ID=UI_Data_CilentID_BAerial;                          //填充操作数据
  }
   framepoint=(unsigned char *)&framehead;
   frametail=Get_CRC16_Check_Sum_UI(framepoint,sizeof(framehead),frametail);
   framepoint=(unsigned char *)&datahead;
...
int Char_ReFresh(String_Data string_Data)
{
	...
	else if(robot_state.robot_id == 105)
  {
	...
	{
	datahead.Sender_ID=UI_Data_RobotID_RDark;
	datahead.Receiver_ID=UI_Data_RobotID_RAerial;
	}
	else if(robot_state.robot_id == 108)
  {
		datahead.Sender_ID=UI_Data_RobotID_BDark;
		datahead.Receiver_ID=UI_Data_RobotID_BAerial;                          //填充操作数据
  }
...
int workshop_communication(UI_Data_t Data)
{
		int i,n;
		...
		UI_Packhead framehead;                       
		UI_Data_Operate datahead;                    //此处不需要更改，和ui相同
				 
		framepoint=(unsigned char *)&framehead;
		framehead.SOF=UI_SOF;
		framehead.Data_Length=6+12;              //长度暂定一字节数据长度
		framehead.Seq=UI_Seq;                        //与UI用同一序号
		framehead.CRC8=Get_CRC8_Check_Sum_UI(framepoint,4,0xFF);
		framehead.CMD_ID=UI_CMD_Robo_Exchange;                   //填充包头数据
   
		datahead.Data_ID = 0x202;//0x200通信包
		if(robot_state.robot_id == 8) //红色方飞手
		{
			datahead.Sender_ID=UI_Data_RobotID_RDark; //无人机飞手发送数据
			datahead.Receiver_ID=UI_Data_RobotID_RAerial;  
		}
		else if(robot_state.robot_id == 108)//蓝色方飞手
		{
			datahead.Sender_ID=UI_Data_RobotID_BDark; //无人机飞手发送数据
			datahead.Receiver_ID=UI_Data_RobotID_BAerial;  
		}
		...
		framepoint=(unsigned char *)&framehead;
		 for(i=0;i<sizeof(framehead);i++)//发送头帧
		{
			UI_SendByte(*framepoint);
			framepoint++;
		}
		framepoint=(unsigned char *)&datahead;//发送数据帧
		for(i=0;i<sizeof(datahead);i++)
		{
			UI_SendByte(*framepoint);
			framepoint++;
		...
		frametail=Get_CRC16_Check_Sum_UI(framepoint,sizeof(Data),frametail);             //CRC16校验
